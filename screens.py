...
def noutrefresh_pad(pad, floor):
	if (floor.max_y - floor.min_y) < curses.LINES-1:
		...
		offset_y = 1
		if floor.max_y - globals.player.y < ((curses.LINES-2)//2):
			new_y = floor.max_y - curses.LINES + 3
		else:
			new_y = globals.player.y - ((curses.LINES-2)//2)
	...
	if (floor.max_x - floor.min_x) < curses.COLS:
		...
		offset_x = 0
		if floor.max_x - globals.player.x  < ((curses.COLS-1)//2):
			new_x = floor.max_x - curses.COLS + 2
		else:
			new_x = globals.player.x-((curses.COLS-1)//2)
...
def draw_messagebar(window):
	window.erase()
	window.addnstr(0, 1, globals.message, curses.COLS-1)
	window.noutrefresh()
...
def inv_menu(window):
	...
	selection = 0
	draw_inv_menu(invwindow, globals.player.inventory, selection)
	invwindow.refresh()
	while ((key:=window.getch())!=ord(" ") and (key != ord("\n"))):
		if(key == curses.KEY_RESIZE):
			...
			selection-=1
		elif((key == curses.KEY_DOWN or key == 107) and selection < len(globals.player.inventory) - 1):
			selection+=1
